...
dir_base="/Users/max/build/PLUMED/md_meta/tests/gromacs"
maxtasks=2
gromacs_prefix="/opt/gromacs-4.0.4/bin/mdrun"
gromacs_prefix_mpi="/opt/gromacs-4.0.4/bin/mdrun_mpi"
#gromacs_version=4
gromacs_version=3
#
 ...
 (
  ...
  do
     ...
     if  [ "${test_file[test_types]}" == "std_out"  ] ; then 
         ...
         if [ "${dir}" == "biasxmd" ] || [ "${dir}" == "ptmetad" ] ; then
     ...
     # added for biasxmd and ptmetad test
     # for gromacs 3
     if [ "${dir}" == "biasxmd" ] || [ "${dir}" == "ptmetad" ] ; then
...
#     #   echo "biasxmd files" $output_file $output_last
     fi
