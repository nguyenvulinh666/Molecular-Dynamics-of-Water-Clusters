...
function to_do_after_patch () {
  sed -e "s/^AMBERBUILDFLAGS=/AMBERBUILDFLAGS=-DAMBER /" ${destination}/src/config.h > tmp.h
...
function to_do_after_revert () {
  sed -e "s/^AMBERBUILDFLAGS=-DAMBER /AMBERBUILDFLAGS=/" ${destination}/src/config.h > tmp.h
  mv tmp.h ${destination}/src/config.h
  grep -q " -DMPI" ${destination}/src/config.h &&
  {
