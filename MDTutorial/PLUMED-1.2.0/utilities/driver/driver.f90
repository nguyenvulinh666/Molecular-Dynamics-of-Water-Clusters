...
!*/
 ...
 logical                  :: go1, go2, go3, go4, do_ext
 logical                  :: dcd_has_cell, cell_input
 character*1              :: ics
 character*80             :: wq_char, line
 character*45             :: filename, pdbout, pdbname, dcdname
 character*8              :: str  
 integer                  :: i, j, m, kk, pbc_on
 integer                  :: ntap, flag, nredux, nfull, ncv
...
! PDB stuff
 ...
 integer, allocatable     :: num(:), numres(:)
...
!! initialization
 ...
 if((.not.(go1.and.go2.and.go3.and.go4))) then 
  write(*,*)
  write(*,*)"USAGE : "
  ...
  write(*,*) 
  write(*,*)"-pdb      : pdb connected to dcd file" 
  write(*,*)"-dcd      : trajectory file" 
  write(*,*)"-plumed   : input file for PLUMED" 
  write(*,*)"-ncv      : number of collective variables"
  write(*,*)"-out      : pdb output filename                                          (optional)" 
  write(*,*)"-interval : extract frames with CV in this interval                      (optional)" 
...
!! check CELL stuff
 ...
 write(*,*)
!! initializing PLUMED 
 call init_metadyn(nfull, occu_full, charge_full, pbc_on, cell_fix, dt,trim(filename)//char(0))
...
!! looping on DCD file
 do
  ...
  read(11,iostat=iostat)(x(m),m=1,nredux)
  if(iostat/=0) exit
  read(11,iostat=iostat)(y(m),m=1,nredux)
  if(iostat/=0) exit
  read(11,iostat=iostat)(z(m),m=1,nredux)
  if(iostat/=0) exit
...
!! check if ORTHORHOMBIC 
   if(cell(4)>0.000000001 .or. cell(5)>0.000000001 .or. cell(6)>0.000000001) then
    write(*,'(a44)') "**** Error !! Only orthorhombic cells are supported in this release."  
    write(*,*)
    stop
   endif
!! printout CELL 
