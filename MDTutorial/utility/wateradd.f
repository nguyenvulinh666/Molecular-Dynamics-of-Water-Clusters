...
c***********************************************************************
      ...
      dimension ox(256),oy(256),oz(256)
      dimension h1x(256),h1y(256),h1z(256)
      dimension h2x(256),h2y(256),h2z(256)
...
c
c     imcon = 10 for ellipsoidal boundary
...
c     read in data from water300K - equilibrated water 
      ...
      do i = 1,naa
        ...
        read(iwt,*)
        read(iwt,*) ox(i),oy(i),oz(i)
        read(iwt,*)
        read(iwt,*) h1x(i),h1y(i),h1z(i)
        read(iwt,*)
        read(iwt,*) h2x(i),h2y(i),h2z(i)
      enddo
c
...
c     inverse of MD cell
      ...
      nsbcll=27
      call spcins
     x   (name,h1x,h1y,h1z,h2x,h2y,h2z,ox,oy,oz,iplus,imax,jmax,kmax,
...
c     link neighbours 
     ...
     x   nix,niy,niz,xxx,yyy,zzz,ssx,ssy,ssz,cell,rcell)
      ...
      dimension ssx(*),ssy(*),ssz(*)
      dimension xxx(*),yyy(*),zzz(*)
      dimension h1x(*),h1y(*),h1z(*)
      dimension h2x(*),h2y(*),h2z(*)
      dimension ox(*),oy(*),oz(*)
...
c     add O to structure
                    ...
                    if(ladd) then
                      ...
                      iplus = iplus + 1
                      k1 = natm+(iplus-1)*3+1
                      if(k1+2.gt.mxatm) call error(200)
...
c**********************************************************************
c
c     dlpoly utility subroutine to remove additional spc waters from
...
c     Truncated octahedral (TO)
      ...
      if(imcon.eq.4) then
c
c     check that Ow is in central box

        do i = natm+1,natm1-2,3
