...
c     
      ...
      subroutine srfrce
     x  (lsolva,lfree,lghost,iatm,ik,engsrp,virsrp,rcut,dlrpot,pindex)
...
c***********************************************************************
      ...
      integer iatm,ik,m,jatm,k,l,kkk,pindex
      ...
      real(8) fy,fz,omega,fsi,fsx,fsy,fsz

      dimension fi(3),fsi(3)
...
c$$$      if(iatm.eq.9160) then
c$$$         write(debugdat,*) ' In srfrce() iatm ',iatm,atmnam(iatm)
c$$$         write(debugdat,*) ' Neighbour list ', ik
c$$$      endif   
...
c$$$        if(iatm.eq.9160) then
c$$$           write(debugdat,*) ' In srfrce() jatm ',jatm,atmnam(jatm)
c$$$        endif   
...
c     update forces always ... original DLPOLY code

              fi(1)=fi(1)+fx
              fi(2)=fi(2)+fy
              fi(3)=fi(3)+fz

c     Update force array on solvent and polymer atoms due to just cross 
c     terms

              if((iatm.le.pindex.and.jatm.gt.pindex).or.
     x (iatm.gt.pindex.and.jatm.le.pindex)) then
                 fsi(1)=fsi(1)+fx
                 fsi(2)=fsi(2)+fy
                 fsi(3)=fsi(3)+fz  
              endif
...
c     update (subtract) forces always ... original DLPOLY code
...
c     terms
              ...
              if((iatm.le.pindex.and.jatm.gt.pindex).or.
     x (iatm.gt.pindex.and.jatm.le.pindex)) then
                 fsxx(jatm)=fsxx(jatm)-fx
                 fsyy(jatm)=fsyy(jatm)-fy
                 fszz(jatm)=fszz(jatm)-fz
              endif
