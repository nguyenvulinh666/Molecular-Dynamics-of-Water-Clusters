...
c***********************************************************************
      ...
      logical lsolva,lfree,lfrmas,lexcite,lswitch,lghost
      ...
      integer minstp,numneb,i,keybpd,mode,nsolva,isolva
...
c     temp scaling interval
      ...
      !! STOCHASTIC & GLE
      logical lstochastic
      integer keystochastic, lgdof,  iseed
      real*8 taus, taub, ascale
...
c     ensemble selection
     ...
     x      mode,taut,taup,ascale,
     x      keystochastic,lgdof,taub,taus)

          ! STOCHASTIC & GLE
        elseif(findstring('stochastic',directive,idum))then
            lstochastic=.true.
            if(findstring('scale',directive,idum))then
              keystochastic=0
              taut=dblstr(directive,69,idum)
              if(idnode.eq.0) write(nrite,
     x            "(/,1x,'Stochastic rescale ',
     x            /,1x,'thermostat relaxation time     ',1p,e12.4)")
     x            taut
            else if(findstring('langevin',directive,idum))then
              keystochastic=1
              taut=dblstr(directive,69,idum)
              if(idnode.eq.0) write(nrite,
     x            "(/,1x,'Stochastic langevin ',
     x          /,1x,'thermostat relaxation time     ',1p,e12.4)")
     x            taut
            else if(findstring('generalized',directive,idum)) then
              keystochastic=2
              lgdof=intstr(directive,69,idum)
              if(idnode.eq.0) write(nrite,
     x               "(/,1x,'Generalized stochastic thermostat ',
     x             /,1x,'number of stoch. DOF     ',1p,i10,
     x             /,1x,'will read from STOCH-A')")
     x             lgdof
              if(findstring('ascale',directive,idum)) then
                 ascale=dblstr(directive,69,idum)
                 if(idnode.eq.0) write(nrite,
     x                "(/,1x,'GLE A matrix scaled by ',1p,e12.4)")
     x                ascale
              endif
            else
              kill=.true.
            ...
            if(findstring('shell',directive,idum)) then
               taus=dblstr(directive,69,idum)
               if(idnode.eq.0) write(nrite,
     x              "(/,1x,'Shell thermostat rel. time ',1p,e12.4)")
     x              taus
            endif
            if(findstring('barostat',directive,idum)) then
               taub=dblstr(directive,69,idum)
               if(idnode.eq.0) write(nrite,
     x              "(/,1x,'Stochastic barostat rel. time ',1p,e12.4)")
     x              taub
            endif
...
c     Initialize random number generator
	    ...
	    if(idnode.eq.0)
     x         write(nrite,*)"random seed ::",iseed
...
c     re-initialise velocities option (regaussing)
     ...
     x        nstbts
        ...
        elseif(findstring('print rdf',directive,idum))then
...
c     unrecognised directive in control file
      ...
      call init_genrand(iseed, idnode)
...
c     
c***********************************************************************
      use random_module, only: mt_read_status
...
CVAM
CVAM      call VTEND(7, ierr)
CVAM
C	read the restart for thermostat, if necessary
      if(keyres.gt.0.and.idnode.eq.0.and.mxlgdof.ne.0)then
         open(7890,file='REVTHERMO',status='OLD',iostat=i)
      	 if (i.eq.0) then
           write(nrite,*) 
     x          " Reading restart file for stochastic thermostat."
            read(7890,*) i,j 
            if (i.ne.mxatms) call error(idnode,3000)
            if (j.ne.mxlgdof) call error(idnode,3001)
            do i=1,mxatms
              read(7890,*) j,grxx(:,i),gryy(:,i),grzz(:,i)
            enddo
            close(7890)
         endif
      endif
c broadcast the stoch. thermo data to all processes. if no restart was made,
c this is a noop, since all the nodes already contain the same copy of parameters
      do i=1,mxlgdof
        call gdbcast(grxx(i,:),mxatms)
        call gdbcast(gryy(i,:),mxatms)
        call gdbcast(grzz(i,:),mxatms)
      enddo
C this is also a good spot to load the PRNG dumps....
      call mt_read_status(idnode,mxnode)
...
c***********************************************************************
      ...
      integer ios, j, ntip4p !!GLE
      real*8, allocatable ::  gleC(:,:), gleSC(:,:), gxv(:)

CVAM
CVAM      call VTBEGIN(6, ierr)
CVAM
...
c     generate starting velocities
      ...
      if(keyres.eq.0)then
        ...
        call gauss(natms,vxx,vyy,vzz,idnode,mxnode)
        
c     init colored thermostat auxiliary variables
        if(mxnode.gt.0) then
          call merge(idnode,mxnode,natms,mxbuff,vxx,vyy,vzz,buffer)
        end if
        
c     init generalized thermostat
        if (mxlgdof .gt. 0) then
          do i=1,mxlgdof
             call gauss(natms,grxx(i,:),gryy(i,:),grzz(i,:),
     x          idnode,mxnode)
             if (mxnode.gt.1) 
     x        call merge(idnode,mxnode,natms,mxbuff,
     x             grxx(i,:),gryy(i,:),grzz(i,:),buffer)
          enddo 
          write(nrite,*) "Initializing colored-noise thermostat DOF."
          open(121,file='STOCH-C',status='OLD',iostat=ios)
          if (ios.ne.0) then  !no STOCH-C ==> init with diagonal temperature
	    grxx=grxx*sqrt(temp*boltz)
            gryy=gryy*sqrt(temp*boltz)
            grzz=grzz*sqrt(temp*boltz)
          else
            allocate(gleC(mxlgdof+1,mxlgdof+1))
            allocate(gleSC(mxlgdof+1,mxlgdof+1))
            allocate(gxv(mxlgdof+1))
            do i=1,mxlgdof+1
              read(121,*) gleC(i,:)  !reads STOCH-C
            enddo
            gleC=gleC*temp*boltz/gleC(1,1)
            write(nrite,*) "Initializing from STOCH-C."
            call cholesky(gleC,gleSC,mxlgdof+1);
            !write(0,*) mxatms, "INIT aux dof!"
            do i=1,mxatms
              gxv(1)=vxx(i)
              do j=1,mxlgdof
                gxv(j+1)=grxx(j,i)
              enddo
              call matmul(gleSC,gxv,gxv)
              vxx(i)=gxv(1)
              do j=1,mxlgdof
                grxx(j,i)=gxv(j+1)
              enddo
...
c          degfre_sol(:)=degrot_sol(:)+dble(rigid_sol(:))*3.d0
...
c        print *, "In quatbook ", degfre,degrot
        if(lsolva)then
...
CVAM
CVAM      call VTEND(145, ierr)
CVAM
      return
      end subroutine intlist
      
      subroutine ensemble_selection
     x  (directive,lens,kill,idnode,keyens,mode,taut,taup,ascale,
     x  keystochastic,lgdof,taub,taus)
c***********************************************************************
c     
