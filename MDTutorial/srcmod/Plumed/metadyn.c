...
int plumed_dd_index(int iat){
   ...
   iat_dd=iat;
#ifdef GMX_MPI
#if defined GROMACS41
   if (DOMAINDECOMP(mtd_data.mcr)) {
     if(!ga2la_home(mtd_data.mcr->dd->ga2la,iat,&iat_dd)){
       iat_dd = -1;
     }
   };
#elif defined GROMACS4
   if (DOMAINDECOMP(mtd_data.mcr)) {
...
void meta_force_calculation(int start,int homenr,real (*pos)[3], real (*force)[3], real box[3][3], real energy)
{
   int i;
   int kk;
...
// if ENERGY CV copy GROMACS forces in colvar.myder and the energy in mtd_data.energy
   if(logical.energy) {
     ...
     for(i=0;i<colvar.natoms[icv_ene];i++){
      iat    = colvar.cvatoms[icv_ene][i];
      iat_dd = plumed_dd_index(iat);
      if(iat_dd>=start && iat_dd<start+homenr){
//       fprintf(stdout," PLUMED :: atom %d  %6.5e \n",iat,sqrt(pow(force[iat_dd][0],2)+pow(force[iat_dd][1],2)+pow(force[iat_dd][2],2)));
       colvar.myder[icv_ene][iat][0]  = -force[iat_dd][0]; 
       colvar.myder[icv_ene][iat][1]  = -force[iat_dd][1];
       colvar.myder[icv_ene][iat][2]  = -force[iat_dd][2]; 
      } else {
       colvar.myder[icv_ene][iat][0]  = 0.;
       colvar.myder[icv_ene][iat][1]  = 0.;
       colvar.myder[icv_ene][iat][2]  = 0.;
      }
     }
   }
...
#elif defined DRIVER || defined RECON_DRIVER
...
void meta_force_calculation_(real *cell, int *istep, real *xxx, real *yyy, real *zzz, real *fxx, real *fyy, real *fzz,real *fsxx, real *fsyy, real *fszz, real *energy,real *energyps, int *protind)
{
...
// in case of ENERGY CV 
 if(logical.energy){ 
   for(i=0;i<colvar.nconst;i++){ 
    if(colvar.type_s[i]==35){
      mtd_data.energy = *energy/mtd_data.eunit;
      icv_ene = i;
      is_en = 1;
    }
// case 40 is using ENERGY_PROTEIN + ENERGY_SOLVENT only
    if(colvar.type_s[i]==40){
      mtd_data.energy = *energyps/mtd_data.eunit;
//      printf("In meta_force %f \n",*energyps);
      icv_ene = i;
      is_enps = 1;
    }
   }
...
// this section might need to be cleaned up
// update forces on protein and solvent atoms seperately
   ...
   if(is_enps == 1){
    ...
    for(i=0;i<*protind;i++){
      ...
      //      printf("FORCE %d %lf %lf %lf\n",i,colvar.myder[icv_ene][i][0],colvar.myder[icv_ene][i][1],colvar.myder[icv_ene][i][2]);
    }

// solvent atoms are updated with forces coming from fsxx,yy,zz
// arrays which include forces on solvent atoms ONLY due to
// the surrounding polymer

    for(i=*protind;i<mtd_data.natoms;i++){
      colvar.myder[icv_ene][i][0] = -fsxx[i]/mtd_data.eunit; 
      colvar.myder[icv_ene][i][1] = -fsyy[i]/mtd_data.eunit;
      colvar.myder[icv_ene][i][2] = -fszz[i]/mtd_data.eunit;
...
// #endif
//  printf("Before Meta force %d %f %f %f\n",99, mtd_data.force[99][0], mtd_data.force[99][1], mtd_data.force[99][2]);

 // printf("Before  %d %f %f %f\n",99, fxx[99],fyy[99],fzz[99]);

#ifdef DL_POLY
 for(i=0;i<mtd_data.natoms;i++){
   
   //  printf("END FORCE %d %f %f %f\n",i, mtd_data.force[i][0], mtd_data.force[i][1], mtd_data.force[i][2]);
...
#ifdef RECON_DRIVER
  int dontmovecolvarfile=1;
#else
  int dontmovecolvarfile=0;
#endif
...
// restore printout
  ...
  // evtl move the old colvar file
      if (firstTime==1 && !logical.append && dontmovecolvarfile!=1 && !logical.restart_abmd){
        sprintf(stringa, "%s.old", mtd_data.hilfilen);
        if(mtd_data.ionode) rename(mtd_data.hilfilen, stringa);
        sprintf(stringa, "%s.old", mtd_data.colfilen);
        if(mtd_data.ionode) rename(mtd_data.colfilen, stringa);
...
#ifdef RECON_DRIVER
  (*ncv)=colvar.nconst;
  (*height)=reconinpt.height;
  (*sizeParam)=reconinpt.iSize;
  (*width)=reconinpt.width;
  for(i=0;i<colvar.nconst;i++){
     periods[i]=0.;
     if( colvar.type_s[i]==4 ) periods[i]=2.*M_PI;
     else if( colvar.type_s[i]==5 ) periods[i]=2.*M_PI;  
     else if( colvar.type_s[i]==34 && colvar.type[i]==2 ) periods[i]=2.*M_PI;
  }
#endif
...
*mod_rij = sqrt(rij[0]*rij[0] + rij[1]*rij[1] + rij[2]*rij[2]);
  ...
  *mod_rij=sqrt(pow(rij[0],2)+pow(rij[1],2)+pow(rij[2],2));
#elif defined (ACEMD) || defined (DRIVER) || defined (AMBER) || defined (STANDALONE) || defined (RECON_DRIVER) || defined (PLUMED_QESPRESSO)
  int i;
