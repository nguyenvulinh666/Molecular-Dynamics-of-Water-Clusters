...
int PREFIX read_dist(char **word, int count, t_plumed_input *input, FILE *fplog)
{
  ...
  double delta = 0.0;
  char string[400];
  char *name_ow1,*name_ow2,*name_ow3;
  int help;
  help=0;
...
void PREFIX import_ow(real **weight ,char* filename , int start, int end , int *indexes )
{
    ...
    if(size!=(end-start)) {
                   ...
                   fprintf(mtd_data.fplog,"|-ERROR: provided mass weights are not sufficient or exceeding the number of atoms. Must provide one weight per atom \n");
                   fprintf(mtd_data.fplog,"|-ERROR: expected number %d   Found number %d \n",end-start,size);
                   EXIT();             
    }
    snew(*weight,size);
    //  reorder and check 
    ...
    for(i=0;i<size;i++){
          ...
          }else{ fprintf(mtd_data.fplog,"|-ERROR: occurring the same atom twice in the weights \n");EXIT(); }
    }
  sfree(ind_tmp); 
  sfree(weight_tmp); 
  sfree(idone);
