...
#if defined (STANDALONE) || defined (LAMMPS_PLUMED) 
    this_ww = hills.wwr*exp(-hills.Vhills/(mtd_data->myboltz*(colvar.wfactor-1.0)*colvar.simtemp));
#else
    this_ww = hills.wwr*exp(-hills.Vhills/(BOLTZ*(colvar.wfactor-1.0)*colvar.simtemp));
#endif
...
void  PREFIX grid_read_fromfile(struct grid_s *grid, int bias)
{
 ...
 int      i, j, with_force, isave, line, m;
 int      header, *index_nd, index_1d;
 char     str1[30], str2[30], str3[30];
 char     *str, stringa[800];
 FILE     *file=NULL;
 double   *ff, *xx, Vgrid, Vp, Vm;
...
// now parsing the grid for potential and forces 
 ...
 while(1){                                     
   if(sscanf(str,"%s %s",str1,str2)>0){
     ...
     while (str != NULL)
     { 
      if(j>=0 && j<tmpgrid.ncv) sscanf(str, "%lf", &(xx[j]));
      if(j==tmpgrid.ncv)        sscanf(str, "%lf", &(Vgrid));
      if(j>tmpgrid.ncv && j<=2*tmpgrid.ncv && with_force==1) sscanf(str, "%lf", &(ff[j-tmpgrid.ncv-1]));
      str =(char *) strtok(NULL, " \t");
      j++;
     }
...
void PREFIX grid_clone(struct grid_s *grid1, struct grid_s *grid2)
{
 ...
 double   xx[nconst_max], ff[nconst_max]; 
