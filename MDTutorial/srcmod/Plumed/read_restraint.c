...
void PREFIX read_restraint(struct mtd_data_s *mtd_data)
{
  ...
  int i, icv, count, exp, iw, nw, ix, iline;
  FILE *file;
...
// Initialize everything for reconnaissance metadynamics
  ...
  reconinpt.nscales=0;
  reconinpt.stride=-10; reconinpt.height=-10.0; reconinpt.width=-10.0; //reconinpt.temp=-10.0;
...
// Reconnaissance metadynamics
      ...
      reconOn=1;
      for(iw=1;iw<nw;iw++){
        if(!strcmp(word[iw],"RESTART")){ reconinpt.restart=1; }
      ...
      for(iw=1;iw<nw;iw++){
        ...
        //   iw++; sscanf(word[iw],"%lf",&uno); reconinpt.temp = (real) uno;
        } else {
      ...
      // reconinpt.dim[reconinpt.nscales]=-10;
      // reconinpt.clusterTol[reconinpt.nscales]=-10.; reconinpt.pcaTol[reconinpt.nscales]=-10.;
      // reconinpt.ntries[reconinpt.nscales]=-10;
      for(iw=1;iw<nw;iw++){
        ...
        // } else if(!strcmp(word[iw],"NTRIES")){
        //   iw++; sscanf(word[iw],"%i",&(reconinpt.ntries[reconinpt.nscales]) );
        // } else if(!strcmp(word[iw],"PCA_DIM")){
        //   iw++;  sscanf(word[iw],"%i",&(reconinpt.dim[reconinpt.nscales]) );
        // } else if(!strcmp(word[iw],"PCA_THRESH")){
        //   iw++; sscanf(word[iw],"%lf",&uno); reconinpt.pcaTol[reconinpt.nscales] = (real) uno;
        // } else if(!strcmp(word[iw],"N_TOL")){
        //   iw++; sscanf(word[iw],"%lf",&uno); reconinpt.clusterTol[reconinpt.nscales] = (real) uno;
        } else {
...
/* Atom position */      
...
#if ! defined (GROMACS4) && ! defined (DL_POLY) && ! defined (AMBER)
      ...
      logical.do_alphabetarmsd=1;
      count++;
    } else if(!strcmp(word[0],"ENERGYPS")) {
#if ! defined (DL_POLY)
          plumed_error("ENERGYPS CV: NOT YET IMPLEMENTED IN THIS CODE");
      #endif
      logical.energy         = 1;
      logical.energyps       = 1;
...
// then we parse the line
      ...
      //if(!read_kappa)plumed_error("WITH CONSTRAINT YOU ALWAYS HAVE TO SPECIFY THE \"KAPPA\" KEYWORD\n");
      ...
      fprintf(mtd_data->fplog, "|-CONSTRAINED SAMPLING OF COLVAR %i AT %f: SPRING=%lf\n\n", icv,cvcnstr.pos[icv-1],cvcnstr.spring[icv-1]/mtd_data->eunit);
         ...
         if(logical.do_steerplan==1){
              fprintf(mtd_data->fplog, "ONLY ONE STEERPLAN IS ALLOWED...BUT YOU MAY HAVE MANY CVS IN THERE!!!\n");
              EXIT();
         }
...
// Call reconnaissance metadynamics setup
   ...
   if( reconOn==1 ){ 
      ...
      double periods[colvar.nconst];
      for(i=0;i<colvar.nconst;i++){
         periods[i]=0.;      
         if( colvar.type_s[i]==4 ) periods[i]=2.*M_PI;
         else if( colvar.type_s[i]==5 ){
            if( colvar.doTrig[i]==0 ){ periods[i]=2.*M_PI; }
            else{ periods[i]=0.; }
         }
      }
      setup_recon( colvar.nconst, periods, tstep, reconinpt, myreconObj);
   }
#endif
...
// checking for conflicts in directive keywords
  if(!logical.do_hills && !logical.commit){
...
// check whether a variable as to be calculated every step or not
  for(i=0;i<colvar.nconst;i++) {
    if(colvar.on[i]||logical.steer[i]||logical.abmd[i]||logical.upper[i]||logical.lower[i]||logical.cnstr[i]) logical.always[i]=1;
    else logical.always[i]=0;
  }
...
int PREFIX  load_steerplan  (char *filename,struct steerplan_s *steerplan, FILE *fplog){
    ...
    fp=fopen(filename,"r");
    if (fp == NULL)
     {fprintf(mtd_data.fplog,"|--UNSUCCESSFULL OPENING FOR FILE %s \n",filename);EXIT(); }; 
    j=0;k=0;
    while(1){
        newline1:
    ...
    for(i=0;i<steerplan->totstages;i++){
        for(j=0;j< steerplan->ncvs;j++){
                   ...
                   for(k=0;k<100;k++){ if(maxcvs[k]==j){ steerplan->actions[i].activecv[j].ncv=k; break;}} 
                   ...
                   if(endcv-j==5){
                       ...
                       if(!strcmp(words[j+4],"CENTRAL")){
                          ...
                          steerplan->actions[jj].activecv[m].type=3;
                           ...
                           fprintf(fplog,"The only types of constraints accepted in the steerplan are CENTRAL POSITIVE NEGATIVE (defalut is CENTRAL of course ;)  \n");  
                           fprintf(fplog,"\n");  
                           EXIT(); 
                       }
                   }else if(endcv-j==4){
