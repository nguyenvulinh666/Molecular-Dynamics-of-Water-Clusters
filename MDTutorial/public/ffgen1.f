...
c     
c     this version developed from the original ffgen.f
c     by Maddalena Venturoli Jan 1997
c
c**********************************************************
...
c     mxtmls = max number of molecular types
      ...
      logical found,hcons,allcons,sub,ldo
      character*4 rname(mxatm),blank4
      character*8 ffnam
...
c     read general proper torsion parameters
 ...
 995  call block(inum,ird2)
      ...
      if(ndi.gt.0) then
        ...
        if(dihtyp.eq.'cos ') then
 	elseif(dihtyp.eq.'harm') then
	else
...
c     read general improper torsion parameters
 ...
 993  call block(inum,ird2)
      ...
      if(nim.gt.0) then
        ...
        if(diityp.eq.'cos ') then
	elseif(diityp.eq.'harm') then
	else
...
c     read non-bonded parameters
	...
	elseif(vdtyp.eq.'lj  ') then 
	elseif(vdtyp.eq.'12-6') then
	else
...
c     taking into account prefactors in potential forms
      ...
      if(vdtyp.eq.'12-6') call units(eunit*vdfac,c12,ilj)
      call units(eunit*vdfac,c6,ilj)
      if(vdtyp.eq.'12-6') call units(eunit*vdfac,c12a,ljx)
      call units(eunit*vdfac,c6a,ljx)
      if(vdtyp.eq.'12-6') call units(eunit*vdfac,c12b,lj14)
...
c     new molecule
      ...
      call bnddata(ird1,ifield,iatm,ibnd,kres,isub,inewsub,mgb,
     x     cb,bo,bnda,bndb,rname,ambnam,ibda,ibdb,ires,x,y,z,qq,
     x     res,resn,rsub)
...
c     
c     joining up non-hydrogens first (iloop=1)
...
c     Improper dihedral force field
            ...
            do i = iatm0+1,iatm1
              ...
              if(ldo) then
                ...
                if(ffnam.eq.'gromos  '.or.ffnam.eq.'charmm  ') then
                  ia = i
                  ib = nexatm(i,1)
                  ic = nexatm(i,2)
                  id = nexatm(i,3)
...
c     amber
                ...
                elseif(ffnam.eq.'gromos  '.or.ffnam.eq.'charmm  ') then
c     
c     gromos and charmm
                    if(irot.lt.2) then
...
c     end of file reached
   ...
   20 ierr = 1
      ...
      subroutine bnddata(ird1,ifield,iatm,ibnd,kres,isub,inewsub,
     x     mgb,cb,bo,bnda,bndb,rname,ambnam,ibda,ibdb,ires,x,y,z,qq,
     x     res,resn,rsub)
     ...
     x      ia,ib,at,an,x1,y1,z1,q1
      ...
      if (ia.gt.0)  then 
         ...
         if(ia.gt.iatm) then
           ...
           if (at(1:3).eq.'OXT')  inewsub = 1   
           if (inewsub.eq.1) then
               if (at(1:3).eq.'N  ') then 
                  isub = isub+1 
                  inewsub = 0 
               endif
           endif
           if(isub.eq.1) rsub(ia)='A'	
           if(isub.eq.2) rsub(ia)='B'	
           if(isub.eq.3) rsub(ia)='C'	
           if(isub.eq.4) rsub(ia)='D'	
           if(isub.eq.5) rsub(ia)='E'	
           if(isub.eq.6) rsub(ia)='F'	
 ...
 10   is = is + 1
      ...
      if(found.and.aimp(is).ne.0.d0) then 
        ...
        if(iloop.eq.2) then
          ...
          write(ifield,'(a4,4i5,5f12.5,4x,i6)') diityp,
     x         ia-iatm0,ib-iatm0,ic-iatm0,id-iatm0,
...
c     too many connections for this atom
              ...
              write(6,*)'too many neighbours for ',i,'(',nami,')',
