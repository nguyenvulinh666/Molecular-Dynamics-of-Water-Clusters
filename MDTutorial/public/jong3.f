      program jong3
c programme to test for clashes and cut out the membrane molecules
c
c the logic of version 1 is that a membrane molecule is removed if at
c least one of its atoms clashes with the protein
c
c copyright P.-L. Chau   2000/01/29
c
c version 1 altered  P.-L. Chau   2006/07/03
c
c the logic of version 2 is that a membrane molecule is removed if and
c only if all its atoms clash with the protein
c
c version 2 copyright P.-L. Chau   2008/04/28
c
c the logic of version 3 is that (1) if a molecule is DMPC, it is removed if
c and only if at least half of its atoms clash with the protein (2) if a
c molecule is water, it is removed even if one of its atoms clashes with the
c protein
c
...
c define rejection fraction, rejfrac. this number defines the fraction of
c clashed atoms in a molecule beyond which the molecule is rejected. it is
c ALWAYS a negative number. e.g., if rejfrac = -0.7, then a molecule can
c have up to 70% of its atoms clashing with the protein and still be retained
c by this programme.
c qualitatively, the more negative the rejection fraction, the more tolerant
c the programme is with clashed molecules
      rejfrac = -0.7
...
c zero variables
      ...
      icount = 1
c read in protein coordinates in DL_POLY format
 100  read(nconfig,'(2a4,i10,6x,a4,x,a3,i6)',end=140) atmnam1(icount)
     +     ,atmnam0(icount),ic,pdbatm(icount),resnam(icount)
     +     ,resnum(icount)
      read(nconfig,*,end=140) px(icount),py(icount),pz(icount)
c test for max and min
...
c read in CONFIG.mb4a file header
      ...
      icount = 1
c read in original membrane atom names in DL_POLY format
 200  read(nconf2,'(a8,i10,6x,a4,x,a3,i6)',end=220) atmnam2(icount),ic
...
c test for clashes
     ...
     +           .and.(dzz(1).lt.djong)) then
               ...
               if (dist2.lt.djong2) then
c stick the first two sets of clashed data in
                  if (mark.lt.2) then
                     mark = mark + 1
...
c go through the current list; if this atom is already listed, then there is
c no need to re-list it
                     do 360 k = 1,mark
 ...
 360                 continue
c if atom not on list already, then go on with distance testing
c set trap for size of jonglist array
...
c and then save the clashed resiude number into a different list
                     jonglist2(mark) = memnum(j)
c once entry is saved, get out of all this mess
                     goto 320
                  endif
               endif
 320           continue
...
c determine cell size
      do 380 i= 1,9
         ...
         if (cell(i).lt.cell2(i)) cell(i) = cell2(i)
 380  continue
...
c print header
      ...
      write(nconfout,'(3f20.12)') cell2(7),cell2(8),cell2(9)
c printout only those atoms that do not clash, firstly ALL protein atoms
      do 400 i = 1,natms
...
c determine the highest memnum in the system
      ...
      do 500 j = 1,maxmemnum
         ...
         indexsum(j) = 0
 500  continue
c logic of this programme: (1) if a molecule is DMPC, it is removed if and
c only if at least half of its atoms clash with the protein (2) if a molecule
c is water, it is removed even if one of its atoms clashes with the protein
c go through jong list to pull out all the molecules which clash with the
c protein
      do 520 i = 1, natms2
...
c if the molecule is water, then even if there is one atom which clashes with
c the protein, the water molecule is rejected
            else
